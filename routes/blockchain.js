...
// Debugging environment variable loading at the start
console.log('Environment Variables at Start:', process.env);

const account1_mnemonic = process.env.ACCOUNT1_MNEMONIC;
const account2_mnemonic = process.env.ACCOUNT2_MNEMONIC;

console.log('Account Mnemonic 1 from user = ' + account1_mnemonic);
console.log('Account Mnemonic 2 from user = ' + account2_mnemonic);

if (!account1_mnemonic || !account2_mnemonic) {
  console.error('Account mnemonics are not defined.');
  process.exit(1);
}

console.log('Recovering accounts using provided secret phrases...');

const recoveredAccount1 = algosdk.mnemonicToSecretKey(account1_mnemonic);
let isValid1 = algosdk.isValidAddress(recoveredAccount1.addr);
console.log('Account 1 recovered: ' + isValid1);
const account1 = recoveredAccount1.addr;
console.log('Account 1 = ' + account1);

const recoveredAccount2 = algosdk.mnemonicToSecretKey(account2_mnemonic);
let isValid2 = algosdk.isValidAddress(recoveredAccount2.addr);
console.log('Account 2 recovered: ' + isValid2);
const account2 = recoveredAccount2.addr;
console.log('Account 2 = ' + account2);

if (!isValid1 || !isValid2) {
  console.error('❌ One or both recovered addresses are invalid.');
  process.exit(1);
}

console.log('Both accounts recovered successfully.');
...
console.log('Environment Variable "ENV" before setting up algod connection: ' + process.env.ENV);
console.log('Algod Token: ' + algodToken);
console.log('Algod Server: ' + algodServer);
console.log('Algod Port: ' + algodPort);

if (!algodToken || !algodServer || !algodPort) {
  console.error('Algod connection details are not defined.');
  process.exit(1);
}

let algodClient;
if (process.env.ENV === 'development') {
  console.log('Setting up algod client for development');
  algodClient = new algosdk.Algodv2(algodToken, algodServer, algodPort);
} else if (process.env.ENV === 'production') {
  console.log('Setting up algod client for production');
  algodClient = new algosdk.Algodv2(algodToken, algodServer, algodPort);
} else {
  console.log('Environment Variable "ENV" is not set to a valid value.');
  process.exit(1);
}

console.log('Algod Client set up successfully.');


module.exports = { algodClient: global.algodClient };
